  ...
  Object.entries(pendingRenders).forEach(([pos, queue]) => {
    ...
    for (const $el of [...children].reverse()) {
      ...
      if (!HasElementTags.includes(elTag))
        continue;
      const props = $el.getAttributeNames().reduce((props2, name) => __spreadProps(__spreadValues({}, props2), { [name]: $el.getAttribute(name) }), {});
  ...
  ArrayableInputs.forEach((key) => {
    ...
    if (typeof val === "object") {
      (Array.isArray(val) ? val : [val]).forEach((entry) => {
        ...
        extras.push(...unpackedEntry.sort((a, b) => a.property === ogKey ? -1 : b.property === ogKey ? 1 : 0));
      });
...
async function normaliseTag(tagName, input) {
  ...
  tag.props = await normaliseProps(tagName, __spreadValues({}, input));
  ...
  if (tag.props.content && Array.isArray(tag.props.content))
    return tag.props.content.map((v) => __spreadProps(__spreadValues({}, tag), { props: __spreadProps(__spreadValues({}, tag.props), { content: v }) }));
  return tag;
}
...
function createHead(options = {}) {
  const head = createHeadCore(__spreadProps(__spreadValues({}, options), {
        ...
        dispose() {
          entries = entries.filter((e) => {
            if (e._i !== activeEntry._i)
              return true;
            _sde = __spreadValues(__spreadValues({}, _sde), e._sde || {});
            e._sde = {};
            updated();
            return false;
          });
...
function createHead2(options = {}) {
  const head = createHead(__spreadProps(__spreadValues({}, options), {
...
function useSeoMeta(input, options) {
  ...
  watchEffect(() => {
    ...
    const _a = resolvedMeta, { title, titleTemplate } = _a, meta = __objRest(_a, ["title", "titleTemplate"]);
    headInput.value = {
      title,
      titleTemplate,
...
function useServerHead(input, options = {}) {
  return serverUseHead(input, __spreadProps(__spreadValues({}, options), { mode: "server" }));
}
...
function ssrRenderTags(tags) {
  ...
  for (const tag of tags) {
    if (tag.tag === "htmlAttrs" || tag.tag === "bodyAttrs") {
      schema[tag.tag] = __spreadValues(__spreadValues({}, schema[tag.tag]), tag.props);
      continue;
    }
