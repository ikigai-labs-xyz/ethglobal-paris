...
import {
  getProvider
} from "./chunk-AI5GKQIY.js";
import {
  getBlockNumber,
  signMessage
} from "./chunk-OXL67OR3.js";
import {
...
init_define_process_env();
var cache = {};
async function getSnapshots(network, snapshot, provider, networks) {
...
setInterval(() => cache = {}, 1e3 * 60 * 60 * 1);

// node_modules/@snapshot-labs/snapshot.js/src/sign/utils.ts
init_define_process_env();
init_lib7();
init_lib3();

// node_modules/@snapshot-labs/snapshot.js/src/sign/eip1271.ts
init_define_process_env();
init_lib();
async function verifyDefault(address, sig, hash2, provider) {
  let returnValue;
  const magicValue = "0x1626ba7e";
  const abi = "function isValidSignature(bytes32 _hash, bytes memory _signature) public view returns (bytes4 magicValue)";
  try {
    returnValue = await call(provider, [abi], [address, "isValidSignature", [arrayify(hash2), sig]]);
  } catch (e) {
...
async function verifyOldVersion(address, sig, hash2, provider) {
  const magicValue = "0x20c13b0b";
  const abi = "function isValidSignature(bytes _hash, bytes memory _signature) public view returns (bytes4 magicValue)";
  const returnValue = await call(provider, [abi], [address, "isValidSignature", [arrayify(hash2), sig]]);
  return returnValue.toLowerCase() === magicValue.toLowerCase();
}
async function verify(address, sig, hash2, network = "1") {
  const provider = getProvider(network);
  if (await verifyDefault(address, sig, hash2, provider))
    return true;
  return await verifyOldVersion(address, sig, hash2, provider);
}
...
async function verify2(address, sig, data, network = "1") {
  const { domain, types, message } = data;
  ...
  return await verify(address, sig, hash2, network);
}
...
var networks_default = {
  ...
  "1666600000": _666600000,
  "1666700000": _666700000,
  "11297108109": _1297108109,
  "11297108099": _1297108099
};

// node_modules/@snapshot-labs/snapshot.js/src/delegationSubgraphs.json
var _10 = "https://subgrapher.snapshot.org/gateway.thegraph.com/api/0f15b42bdeff7a063a4e1757d7e2f99e/deployments/id/QmXZiV6S13ha6QXq4dmaM3TB4CHcDxBMvGexSNu9Kc28EH";
var _15 = "https://subgrapher.snapshot.org/api.thegraph.com/subgraphs/name/snapshot-labs/snapshot-goerli";
var _07 = "https://subgrapher.snapshot.org/api.thegraph.com/subgraphs/name/snapshot-labs/snapshot-optimism";
var _66 = "https://subgrapher.snapshot.org/api.thegraph.com/subgraphs/name/snapshot-labs/snapshot-binance-smart-chain";
var _003 = "https://subgrapher.snapshot.org/api.thegraph.com/subgraphs/name/snapshot-labs/snapshot-gnosis-chain";
var _372 = "https://subgrapher.snapshot.org/api.thegraph.com/subgraphs/name/snapshot-labs/snapshot-polygon";
var _502 = "https://subgrapher.snapshot.org/api.thegraph.com/subgraphs/name/snapshot-labs/snapshot-fantom";
var _21612 = "https://subgrapher.snapshot.org/api.thegraph.com/subgraphs/name/snapshot-labs/snapshot-arbitrum";
...
var ApprovalVoting = class {
  ...
  static isValidChoice(voteChoice, proposalChoices) {
    return Array.isArray(voteChoice) && voteChoice.every((choice) => (proposalChoices == null ? void 0 : proposalChoices[choice - 1]) !== void 0) && voteChoice.length === new Set(voteChoice).size;
  }
  getValidVotes() {
...
function irv(ballots, rounds) {
  ...
  const votes = Object.entries(ballots.reduce((votes2, [v], i, src) => {
...
function getScoresMethod(votes, proposal) {
  ...
  return proposal.choices.map((choice, i) => finalRound.filter((res) => Number(res[0]) === i + 1).reduce((a, b) => a + b[1][0], 0));
}
var RankedChoiceVoting = class {
  constructor(proposal, votes, strategies, selected) {
    this.proposal = proposal;
    this.votes = votes;
    this.strategies = strategies;
  ...
  static isValidChoice(voteChoice, proposalChoices) {
    return Array.isArray(voteChoice) && voteChoice.every((voteChoice2) => (proposalChoices == null ? void 0 : proposalChoices[voteChoice2 - 1]) !== void 0) && voteChoice.length === new Set(voteChoice).size && voteChoice.length > 0 && voteChoice.length === proposalChoices.length;
  }
  getValidVotes() {
...
var WeightedVoting = class {
  ...
  getScores() {
    const results = this.proposal.choices.map((choice, i) => this.getValidVotes().map((vote) => weightedPower(i, vote.choice, vote.balance)).reduce((a, b) => a + b, 0));
  ...
  getScoresByStrategy() {
    const results = this.proposal.choices.map((choice, i) => this.strategies.map((strategy, sI) => this.getValidVotes().map((vote) => weightedPower(i, vote.choice, vote.scores[sI])).reduce((a, b) => a + b, 0))).map((arr) => arr.map((pwr) => [pwr]));
    const validScoresTotal = this.getValidVotes().reduce((a, b) => a + b.balance, 0);
    return results.map((res, i) => this.strategies.map((strategy, sI) => percentageOfTotal(0, results[i][sI], results.flat(2))).map((p) => [validScoresTotal / 100 * p]).flat());
  }
  getScoresTotal() {
    return this.votes.reduce((a, b) => a + b.balance, 0);
  }
...
async function multicall(network, provider, abi, calls, options) {
  ...
  const multi = new Contract(networks_default[network].multicall, multicallAbi, provider);
  ...
  try {
    ...
    Array.from(Array(pages)).forEach((x, i) => {
      ...
      promises.push(multi.aggregate(callsInPage.map((call2) => [
        call2[0].toLowerCase(),
    ...
    return results.map((call2, i) => itf.decodeFunctionResult(calls[i][1], call2));
  } catch (e) {
    return Promise.reject(e);
  }
}
...
async function getScores(space, strategies, network, addresses, snapshot = "latest", scoreApiUrl = "https://score.snapshot.org/api/scores") {
  try {
    ...
    const res = await (0, import_cross_fetch.default)(scoreApiUrl, {
      method: "POST",
...
async function getVp(address, network, strategies, snapshot, space, delegation, options) {
  ...
  const res = await (0, import_cross_fetch.default)(options.url, init);
...
async function validate(validation, author, space, network, snapshot, params, options) {
  if (!options)
    options = {};
  if (!options.url)
    options.url = "https://score.snapshot.org";
...
async function getEnsTextRecord(ens, record, network = "1") {
  const ensResolvers = networks_default[network].ensResolvers || networks_default["1"].ensResolvers;
  const ensHash = (0, import_eth_ens_namehash.hash)((0, import_eth_ens_namehash.normalize)(ens));
  const provider = getProvider(network);
  const result = await multicall(network, provider, ENS_RESOLVER_ABI, ensResolvers.map((address) => [address, "text", [ensHash, record]]));
  return result.flat().find((r) => r) || "";
}
async function getSpaceUri(id, network = "1") {
  try {
...
async function getSpaceController(id, network = "1") {
...
async function getDelegatesBySpace(network, space, snapshot = "latest") {
  if (!delegationSubgraphs_default[network]) {
  ...
  while (true) {
    ...
    const pageResult = await subgraphRequest(delegationSubgraphs_default[network], params);
...
//# sourceMappingURL=chunk-7SUWGD7G.js.map
